...
> 截图放在 `docs/screenshots/` 目录下，替换同名文件即可更新。
...
![首页看板](docs/screenshots/01-dashboard.png)

一页只有主卡这一个大数字，视线有落点。右侧待办面板**按紧急程度固定排序** ——
不按数量排，因为按数量排的顺序会随数据跳动，用户每次都得重新找。
页面上唯一由模型生成的内容是那块 AI 摘要，用左侧一道 `--signal` 竖条标识身份。

#### 设备管理

![设备管理](docs/screenshots/02-device-list.png)

状态分布和分类统计两张图放在列表上方，筛选条件和数字对得上（图表的数字和指标卡
取自**同一份 group by 结果**，分开查就会出现"卡片说 6 台、饼图说 7 台"）。
操作列收进了下拉菜单 —— 一台设备可以有借用、归还、报修、调拨、报废、恢复六种动作，
平铺出来会把操作列撑爆。默认只显示 9 列（1105px，普通屏幕放得下），
型号、厂商、序列号等低频字段按需打开。

#### 智能故障诊断

![智能故障诊断](docs/screenshots/04-diagnosis.png)

左边是**依据资料**（手册片段 / 相似历史工单 / 建议备件，备件带当前库存），右边是流式生成区。
资料和生成分开显示是刻意的：小模型面对"没有资料"时的默认行为是编一个像样的答案，
把检索到的原文摊开让人能自己核对，比指望模型自觉可靠得多。

#### 资产标签

![资产标签](docs/screenshots/03-device-detail.png)

设备档案页上的二维码弹窗，标签里放的是**档案页的完整地址**，手机相机直接扫就能打开。
二维码**刻意不跟随主题**：它是给机器扫的，必须浅底深码 —— 暗色主题下反过来渲染
（浅码压深底）很多扫码器认不出来，而扫不出来的二维码完全没用。

### 管理后台

管理后台是另一套外壳：**深色左侧竖栏 + 顶部一道强调条**，和员工工作台在结构上就不一样，
一眼能看出是换了个系统。两者共用一个登录态、一份构建、一套组件 ——
区域判定用**路径前缀**而不是角色，因为管理员同时也是员工，他在哪边取决于此刻在哪个页面。

#### 系统概览

![系统概览](docs/screenshots/05-console-home.png)

「需要关注」四项异常指标放在最前面，规模数字放后面。纯罗列规模（"用户 12 个、角色 2 个"）
看一次就没价值了 —— 它不会变、也不需要处理；真正值得每天扫一眼的是**异常的那几个**。

#### 用户管理

![用户管理](docs/screenshots/06-user-management.png)

组合筛选（用户名 / 角色 / 状态 / 创建时间范围）、批量删除 / 改状态 / 重置密码、导出 xlsx。
批量操作**不做"全成功或全失败"**：选 10 个里有 1 个受保护，就跳过那 1 个、其余照做，
返回"成功几个 + 哪几个被跳过、为什么"。全部回滚不合理（其余 9 个明明能处理），
静默跳过也不好（用户以为处理干净了）。

#### 设备知识库

![设备知识库](docs/screenshots/07-knowledge.png)

RAG 管道的入口：上传手册/厂商资料/维修案例后，系统会解析、切分、转成向量存进库里，
检索时按语义相似度找相关段落。页面下方的检索试验面板会显示每条命中的相似度分数 ——
它同时是**诊断门槛的调节工具**：问一个你知道答案就在库里的问题，看"最相近的一段"离门槛差多少，
差得不多就说明门槛定高了。

#### 暗色主题

![暗色主题](docs/screenshots/08-dark.png)

亮暗两套 token 从第一天就一起设计（后补的成本高得多）。工业控制台方向：深色轨道 + 浅色工作区、
状态指示灯、等宽数字、铭牌式编号。所有"前景压背景"的组合都算过对比度，不是凭感觉调的 ——
比如 `--ink-3` 的适用范围只有 surface / canvas / hover 三个底，压在更暗的淡底上会掉到 4.2 左右。
